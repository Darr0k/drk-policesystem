...
RegisterNetEvent("DRKDispatch:Client:OnPlayerLoaded", function()
    isLoggedIn = true
    PlayerData = DRKDispatch.Functions.GetPlayerData()
    PlayerJob = DRKDispatch.Functions.GetPlayerData().job
end)

RegisterNetEvent('DRKDispatch:Client:OnPlayerUnload', function()
	PlayerData = {}
    isLoggedIn = false
    currentCallSign = ""
...
function GetPedGender()
    local gender = "Male"
    if DRKDispatch.Functions.GetPlayerData().charinfo.gender == 1 then gender = "Female" end
    return gender
end
...
RegisterNetEvent('dispatch:manageNotifs', function(sentSetting)
    ...
    if wantedSetting == "on" then
        ...
        DRKDispatch.Functions.Notify("Dispatch enabled", "success")
    elseif wantedSetting == "off" then
        disableNotis = true
        disableNotifSounds = true
        DRKDispatch.Functions.Notify("Dispatch disabled", "success")
    elseif wantedSetting == "mute" then
        disableNotis = false
        disableNotifSounds = true
        DRKDispatch.Functions.Notify("Dispatch muted", "success")
    else
        DRKDispatch.Functions.Notify('Please choose to have dispatch as "on", "off" or "mute".', "success")

    end
end)

RegisterNetEvent('dispatch:clNotify', function(sNotificationData, sNotificationId, sender)
	-- --print(sNotificationData ~= nil, LocalPlayer.state['isLoggedIn'], IsValidJob(sNotificationData['job']), CheckOnDuty(), disableNotis, sNotificationData.origin ~= nil, json.encode(sNotificationData['job']))
		...
		if IsValidJob(sNotificationData['job']) and CheckOnDuty() then
            if not disableNotis then
				if sNotificationData.origin ~= nil then
                    local PlayerJob = DRKDispatch.Functions.GetPlayerData().job
                    local ped = PlayerPedId()
                    local areaName = Library:GetPlayerArea(ped)
					sNotificationData.area = areaName
					--print(sNotificationData.area)
                    SendNUIMessage({
                        action = 'NewCall',
                        info = sNotificationData,
                        timer = Library:ConvertToSecond(5),
                        IsPolice = Library:IsPolice(PlayerJob.name)
                    })
				end
			end
        end
...
RegisterNetEvent("drk-policesystem:client:AddCallBlip")
AddEventHandler("drk-policesystem:client:AddCallBlip", function(coords, data, blipId)
...
RegisterNetEvent("drk-policesystem:client:Explosion", function(data)
	exports["drk-policesystem"]:Explosion()
end)

RegisterNetEvent("drk-policesystem:client:removeCallBlip", function(blipId)
	RemoveBlip(blips[blipId])
end)

RegisterNetEvent("drk-policesystem:client:clearAllBlips", function()
	for k, v in pairs(blips) do
		RemoveBlip(v)
	end
	DRKDispatch.Functions.Notify('All dispatch blips cleared', "success")
end)

RegisterCommand('+openControl', function()
	local PlayerJob = DRKDispatch.Functions.GetPlayerData().job
	DRKDispatch.Functions.TriggerCallback('drk-policesystem:server:getActiveCopsNumber', function(result)
		if (Library:IsPolice(PlayerJob.name) or Library:IsEms(PlayerJob.name)) then
			SendNUIMessage({
				action = 'OpenAlertControl',
				activeCops = result
			})
...
RegisterNUICallback('HideControl', function()
