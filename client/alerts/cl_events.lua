...
local function VehicleShooting(vehdata)
    local vehicle = DRKDispatch.Functions.GetClosestVehicle()
    local vehdata = vehicleData(vehicle)
    local currentPos = GetEntityCoords(PlayerPedId())
    local locationInfo = getStreetandZone(currentPos)
    local heading = getCardinalDirectionFromHeading()
...
RegisterNetEvent('drk-policesystem:client:drugsale', function()
    DrugSale()
end)
...
local function OfficerDown()
    local plyData = DRKDispatch.Functions.GetPlayerData()
    local currentPos = GetEntityCoords(PlayerPedId())
    local locationInfo = getStreetandZone(currentPos)
    local callsign = DRKDispatch.Functions.GetPlayerData().metadata["callsign"]
    TriggerServerEvent("dispatch:server:notify", {
        ...
        icon = './images/injuriy.svg'
    })
end

exports('OfficerDown', OfficerDown)

RegisterNetEvent("drk-policesystem:client:officerdown", function ()
    OfficerDown()
end)
...
RegisterNetEvent("drk-policesystem:client:emsdown", function ()
    EmsDown()
end)
